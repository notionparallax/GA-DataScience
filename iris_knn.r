...
library(class)
library(ggplot2)
#library(DMwR) #students can check it out
...
pairs(data[1:4],main="Iris Data (red=setosa,green=versicolor,blue=virginica)", pch=21,
      bg=c("red","green3","blue")[unclass(iris$Species)]) #Base R no packages

plotmatrix(data[1:4]) # using ggplot2 package

ggpairs(data , colour = 'Species' , alpha=0.2) #using the recommended GGally package
